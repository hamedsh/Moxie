...
async def test_create_rule(db_session: AsyncSession) -> None:
    ...
    await rule_crud.create_rule(db_session, test_rule)
...
async def test_disable_rule(db_session: AsyncSession) -> None:
    ...
    updated_rule: RuleModel = await rule_crud.disable_rule(db_session, rule.id, RuleStatusChange(enable=False))
...
async def test_reduce_mock_count(
    ...
    await rule_crud.reduce_use_count(db_session, rule)
...
async def test_delete_rule(db_session: AsyncSession) -> None:
    ...
    await rule_crud.delete_rule(db_session, rule.id)
...
async def test_search_rules_if_exist(db_session: AsyncSession) -> None:
    ...
    result = await rule_crud.search_rule(db_session, test_method, 'test_url/api/api_v1/id')
...
async def test_search_rules_if_not_exist(db_session: AsyncSession) -> None:
    ...
    result = await rule_crud.search_rule(db_session, 'NOT_EXIST', test_url)
...
async def test_get_all_rules(db_session: AsyncSession) -> None:
    ...
    result = await rule_crud.get_all_rules(db_session)
